...
export default function Header() {
  ...
  const handleSubmit = (e) => {
    ...
    e.preventDefault();
    // Keep and update the current search params
    const urlParams = new URLSearchParams(window.location.search);
    urlParams.set('searchTerm', searchTerm);
    const searchQuery = urlParams.toString();
    // Navigate user to updated url
    navigate(`/search/?${searchQuery}`);
  };
  // Concurrently update the search term in the search form with the url
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    ...
    <header className='bg-slate-200 shadow-md'>
      ...
      <div className='flex justify-between items-center max-w-6xl mx-auto p-3'>
        ...
        <h1 className='font-bold text-sm sm:text-xl flex flex-wrap'>
          <span className='text-slate-500'>Convenient</span>
          <span className='text-slate-700'>Lodge</span>
        </h1>
        </Link>
        {/* Search Bar */}
